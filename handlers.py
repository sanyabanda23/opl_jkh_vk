...
@vk_bot.on.message(MyRule(), PayloadABCRule('ltpt'))
async def opl_lt_pt_pok(message: Message):
    try:
        await vk_bot.state_dispenser.delete(message.peer_id)
    except KeyError:
        pass  # Состояние не найдено — игнорируем
    await message.answer('Укажи показания счетчика электроэнергии.')
    await vk_bot.state_dispenser.set(message.peer_id, Opl_lt_pt.POK_LT)

@vk_bot.on.message(MyRule(), state=Opl_lt_pt.POK_LT)
async def opl_lt_pt_preparetion(message: Message):
    ctx.get("pok_lt", message.text)        
    connection = con.connect(
              host=settings.con_sql[0],
              user=settings.con_sql[1],
    ...
    if input_value[0] is True:
        await message.answer(text.question_pay_lt.format(input_value[2], input_value[1]), keyboard=kb.yes_no_kb())
        await vk_bot.state_dispenser.set(message.peer_id, Opl_lt_pt.PREPARATION)
    else:
        await message.answer(text.falling_pay, keyboard=kb.opl_zkh_pt())

@vk_bot.on.message(MyRule(), PayloadABCRule('yes'), state=Opl_lt_pt.PREPARATION)
async def opl_lt_pt(message: Message):        
    if driver_jkh.oplata_lt_yes():    
        rekviz = utils.get_info_from_chek()
        if rekviz:
            num = rekviz[0]
...
@vk_bot.on.message(MyRule(), PayloadABCRule('no'), state=Opl_lt_pt.PREPARATION)
async def opl_lt_pt(message: Message):
    await message.answer('Укажи сумму, которую собираешься оплатить.')
    await vk_bot.state_dispenser.set(message.peer_id, Opl_lt_pt.SUMM)

@vk_bot.on.message(MyRule(), state=Opl_lt_pt.SUMM)
async def opl_lt_pt(message: Message):
    data_summ = message.text
    connection = con.connect(
              host=settings.con_sql[0],
...
@vk_bot.on.message(MyRule(), PayloadABCRule('gzfr'))
async def opl_gz_fr_preparetion(message: Message):
    try:
        await vk_bot.state_dispenser.delete(message.peer_id)
    except KeyError:
        pass  # Состояние не найдено — игнорируем
    ...
    if input_value[0] is True:
        await message.answer(text.question_pay.format(input_value[1]), keyboard=kb.yes_no_kb())
        await vk_bot.state_dispenser.set(message.peer_id, Opl_gz_fr.PREPARATION)
    else:
        await message.answer(text.falling_pay, keyboard=kb.opl_zkh_fr())

@vk_bot.on.message(MyRule(), PayloadABCRule('yes'), state=Opl_gz_fr.PREPARATION)
async def opl_gz_fr_preparetion(message: Message):        
    if driver_jkh.oplata_gz_yes():    
        rekviz = utils.get_info_from_chek()
        if rekviz:
...
@vk_bot.on.message(MyRule(), PayloadABCRule('no'), state=Opl_gz_fr.PREPARATION)
async def opl_gz_fr(message: Message):
    await message.answer('Укажи сумму, которую собираешься оплатить.')
    await vk_bot.state_dispenser.set(message.peer_id, Opl_gz_fr.SUMM)

@vk_bot.on.message(MyRule(), state=Opl_gz_fr.SUMM)
async def opl_gz_fr(message: Message):
    data_summ = message.text
    connection = con.connect(
              host=settings.con_sql[0],
    ...
    try:
        ...
        connection.close()
    await message.answer(text.preparation_pay)
    input_value = driver_jkh.oplata_gz(inn=inn, l_sch=l_sch, schet=schet, bik=bik, pok=pok, summ=data_summ)
    if input_value[0] is True:
        await message.answer(text.question_pay.format(input_value[1]), keyboard=kb.yes_no_kb())
        await vk_bot.state_dispenser.set(message.peer_id, Opl_gz_fr.PREPARATION)
    else:
